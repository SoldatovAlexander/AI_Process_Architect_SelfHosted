...
import { api, ApiError, downloadExport, downloadProjectArchive, type AgentTarget, type AppSpecTarget, type ExportFormat } from '../api'
...
export function WorkspaceScreen({ projectId, onBack }: { projectId: string; onBack: () => void }) {
  ...
  return (
    <div className={`workspace ${seedProject ? 'workspace--interview-first' : ''}`} data-help-topic="project">
      ...
      {showExport && <ExportModal project={project} format={exportFormat} setFormat={setExportFormat} appTarget={appSpecTarget} setAppTarget={setAppSpecTarget} n8nTarget={n8nTarget} setN8nTarget={setN8nTarget} includeN8nGuide={includeN8nGuide} setIncludeN8nGuide={setIncludeN8nGuide} agentTarget={agentTarget} setAgentTarget={setAgentTarget} readiness={readiness} agentReadiness={agentReadiness} roundTrip={hasN8nSource} perspective={project.current_revision.perspective} busy={busy} onClose={() => setShowExport(false)} onDownload={async () => { setBusy(true); try { await downloadExport(project, exportFormat, n8nTarget, appSpecTarget, agentTarget, includeN8nGuide, hasN8nSource) } catch (reason) { setError(workspaceError(reason, t)) } finally { setBusy(false) } }} />}
...
function ExportModal({ project, format, setFormat, appTarget, setAppTarget, n8nTarget, setN8nTarget, includeN8nGuide, setIncludeN8nGuide, agentTarget, setAgentTarget, readiness, agentReadiness, roundTrip, perspective, busy, onClose, onDownload }: {
