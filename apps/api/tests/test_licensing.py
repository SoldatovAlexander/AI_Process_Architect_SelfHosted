...
def test_license_control_allows_only_private_docker_http_endpoint():
    ...
    assert _base_url(settings) == "http://license-control:8090"

    for url in ("http://issuer.example", "http://127.0.0.1:8090", "ftp://license-control"):
        with pytest.raises(LicenseControlError):
            _base_url(Settings(license_control_plane_url=url, license_control_plane_operator_token="operator-token"))
